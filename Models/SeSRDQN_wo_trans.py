...
class SeSRDQN_wo_trans(object):
    ...
    def _build_network(self):
        ...
        # should not add lambda_d to the difficulty loss
        self.loss += tf.reduce_mean(tf.square(self.v_difficulty - self.difficulty_label))
    ...
    def get_global_attention_weights(self):
        data = [[[0] * self.n_s], [[0] * self.pool_size], [0], [0], [[0] * self.n_s]]
        sal_weights, dif_weights = self.sess.run((tf.nn.softplus(self.weights["%s_salary_skill_action_value" % self.name]), tf.nn.softplus(self.weights["%s_difficulty_skill_action_value" % self.name])), self.get_dict(data, train=False))
        return sal_weights, dif_weights

    def get_salary_skill_embeddings(self):
        data = [[[0] * self.n_s], [[0] * self.pool_size], [0], [0], [[0] * self.n_s]]
        embeddings = self.sess.run((self.weights["%s_salary_embedding" % self.name]), self.get_dict(data, train=False))
        return embeddings

    def get_difficulty_skill_embeddings(self):
        data = [[[0] * self.n_s], [[0] * self.pool_size], [0], [0], [[0] * self.n_s]]
        embeddings = self.sess.run((self.weights["%s_difficulty_embedding" % self.name]), self.get_dict(data, train=False))
        return embeddings