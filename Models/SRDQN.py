...
class SRDQN(object):
    ...
    def get_dict(self, data, train=True):
        feed_dict = {
            ...
            self.deep_salarybase_dropout: self.dropout_salary_deep_feed if train else [1] * len(self.dropout_salary_deep_feed)
        }
        return feed_dict
    ...
    def _build_network(self):
        ...
        self.input_actpool = tf.placeholder(tf.int32, shape=[None, self.pool_size], name="%s_input_actpool" % self.name) # -1, pool_size

        # ------------------------ 读embedding -------------------
        emb = tf.Variable(np.random.normal(size=(self.n_s, self.embsize)),
                          dtype=np.float32, name="%s_embedding" % self.name) # n_s, embsize 每个skill对应一个embedding
        self.weights["%s_embedding" % self.name] = emb # embedding is copied into weights

        input_emb = tf.matmul(self.input_state, emb) # -1, embsize 每次数据state的embedding
        input_emb = tf.tile(tf.reshape(input_emb, [-1, 1, self.embsize]), [1, self.pool_size, 1]) # -1, pool_size, embsize  复制了pool_size次
        act_emb = tf.nn.embedding_lookup(emb, self.input_actpool) # -1, pool_size, embsize

        # ------------------------ salary部分 ---------------------
        salary_in = tf.concat((input_emb, act_emb), axis=2)  # -1, pool_size, 2 * embsize
        deep_in = tf.reshape(salary_in, [-1, self.embsize * 2]) # -1 (num * pool_size), 2 * embsize
        ...
        self.b_salary_nxt, _ = self._fc(y_deep, self.salary_deep_layers[-1], 1, name="%s_salary_qbase" % self.name, l2_reg=0.0,
                                        activation=None, bias=True, sparse=False)
        self.q_salary = self.b_salary_nxt + self.s_salary
        ...
        self.q_easy, _ = self._fc(y_deep, self.easy_deep_layers[-1], 1, name="%s_easy_output" % self.name, l2_reg=0.0,
                                  activation=None, bias=True, sparse=False)
        self.q_easy = tf.reshape(self.q_easy, [-1, self.pool_size]) # -1, pool_size

        # --------------------- action预测 ------------------------------
        self.q = self.lambda_d * self.q_easy + self.q_salary

        self.v_place = tf.argmax(self.q, 1) # pool_size中选取最大的

        onehot_action_nxt = tf.one_hot(self.v_place, self.pool_size)  # [0, 1] one-hot vector
        self.v_skill = tf.cast(tf.reduce_sum(tf.multiply(onehot_action_nxt, tf.cast(self.input_actpool, np.float32)), 1), np.int32) # -1, 1
        ...
        self.loss += penalty_loss # penalty_loss is actually 0
        return
