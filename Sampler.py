...
class BestStrategyPoolSampler(object):
    ...
    def sample_top_candidates(self, state, pool=None):
        if pool is None:
            pool = get_act_pool(state, self.n_a, self.relational_lst, self.pool_size)
        (q_salary, q_easy), act_lst, q = self.Qa.estimate_maxq_batch_sample(state, pool)
        return (q_salary, q_easy), act_lst, q

    # view weights 
    def sample_with_batch(self, state, pool=None):
        if pool is None:
            pool = get_act_pool(state, self.n_a, self.relational_lst, self.pool_size)
        q_ret, s_ret, attention_salary, salary_prototypes, attention_difficulty, difficulty_prototypes, salary_action_value, difficulty_action_value, salary_mapped, original_salary_mapped, chosen_action, difficulty_mapped = self.Qa.estimate_maxq_action_with_prototypes(state, pool)
        return s_ret, q_ret, attention_salary, salary_prototypes, attention_difficulty, difficulty_prototypes, salary_action_value, difficulty_action_value, salary_mapped, original_salary_mapped, chosen_action, difficulty_mapped

    # view weights given a skill
    def sample_with_batch_with_skill(self, state, skill):
        q_ret, s_ret, attention_salary, salary_prototypes, attention_difficulty, difficulty_prototypes, salary_action_value, difficulty_action_value, salary_mapped, original_salary_mapped, chosen_action, difficulty_mapped = self.Qa.estimate_maxq_action_with_prototypes_with_skill(state, skill)
        return s_ret, q_ret, attention_salary, salary_prototypes, attention_difficulty, difficulty_prototypes, salary_action_value, difficulty_action_value, salary_mapped, original_salary_mapped, chosen_action, difficulty_mapped

    # view internal variables
    def sample_with_internal_things(self, state, pool=None):
        if pool is None:
            pool = get_act_pool(state, self.n_a, self.relational_lst, self.pool_size)
        loss, encoder_loss, div_loss, freq_loss = self.Qa.evaluate_internal_value_functions(state, pool)
