...
from Models.ProSeNet import ProSeNet
import os
...
class OnPolicyTrainer(object):
        ...
        for it in tqdm(range(n_batch)):
            ...
            if it != 0 and it % verbose_batch == 0:
                ...
                if save_path is None:
                    self.Qnet.save(HOME_PATH + "data/model/%s_ProSeNet_sal_%s_dif_%s_info_%s_freq_%s_div_%s_enc_%s_%s" %(direct_name, salary_prototype, difficulty_prototype, lambda_info, lambda_freq, lambda_div, lambda_enc, env_params))
                else:
            ...
            for t in range(T):
                ...
                self.memory.store((state_pre, s, (easy, salary - salary_pre), [[0] * self.n_s]))
                salary_pre = salary
            ...
            if self.memory.get_size() > batch_size * 10:
                ...
                v_value, s_value, loss, info_loss, div_loss, freq_loss, enc_loss = self.Qnet.run(data_batch, batch_size, train=True)
    ...
    def transform_train_batch(self, data_batch, batch_size):
        ...
        return data_state, [[skill] * self.pool_size for skill in data_skill], data_salary, data_easy, [[0] * self.n_s]
...
parser.add_argument('--lambda_info', type=float, default=0.01)
parser.add_argument('--lambda_freq', type=float, default=0.01)
parser.add_argument('--lambda_div', type=float, default=0.1)
parser.add_argument('--lambda_enc', type=float, default=0.0)
...
if __name__ == "__main__":
    wandb.init(
       project = "SkillRec",
       tags=["ProSeNet", str(args.salary_prototype), str(args.difficulty_prototype), str(args.lambda_info), str(args.lambda_freq), str(args.lambda_div), str(args.lambda_enc), str(args.lambda_d), str(args.beta), str(args.pool_size)],
       entity="yangji721",
       config = wandb.config,
       # config = config,
       mode="disabled",
    ...
    Qa = ProSeNet(n_skill, lambda_info = lambda_info, lambda_div= lambda_div, lambda_enc = lambda_enc, lambda_freq = lambda_freq, verbose=1, learning_rate=lr, lambda_d=env_params['lambda_d'], embsize=emb_size, 
    ...
    # ----------------- Model Saving -----------------------
    Qa.save(HOME_PATH + "data/model/%s_ProSeNet_sal_%s_dif_%s_info_%s_freq_%s_div_%s_enc_%s_%s" %(direct_name, salary_prototype, difficulty_prototype, lambda_info, lambda_freq, lambda_div, lambda_enc, env_params))