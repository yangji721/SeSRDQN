...
from Models.interpretableDQN import interpretableDQN
import os
        ...
        for it in tqdm(range(n_batch)):
            ...
            if it != 0 and it % verbose_batch == 0:
                ...
                if save_path is None:
                    self.Qnet.save(HOME_PATH + "data/model/%s_interpretableDQN_sal_%s_dif_%s_info_%s_freq_%s_div_%s_enc_%s_%s" %(direct_name, salary_prototype, difficulty_prototype, lambda_info, lambda_freq, lambda_div, lambda_enc, env_params))
                else:
...
parser.add_argument('--lambda_freq', type=float, default=0.001)
...
if __name__ == "__main__":
    ...
    Qa = interpretableDQN(n_skill, lambda_info = lambda_info, lambda_div= lambda_div, lambda_enc = lambda_enc, lambda_freq = lambda_freq, verbose=1, learning_rate=lr, lambda_d=env_params['lambda_d'], embsize=emb_size, 
    ...
    # ----------------- Model Saving -----------------------
    Qa.save(HOME_PATH + "data/model/%s_interpretableDQN_sal_%s_dif_%s_info_%s_freq_%s_div_%s_enc_%s_%s" %(direct_name, salary_prototype, difficulty_prototype, lambda_info, lambda_freq, lambda_div, lambda_enc, env_params))