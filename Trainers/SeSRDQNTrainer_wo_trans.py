...
from Models.SeSRDQN_wo_trans import SeSRDQN_wo_trans
import os
...
class OnPolicyTrainer(object):
        ...
        for it in tqdm(range(n_batch)):
            ...
            if it != 0 and it % verbose_batch == 0:
                ...
                if save_path is None:
                    self.Qnet.save(HOME_PATH + "data/model/%s_SeSRDQN_wo_trans_sal_%s_dif_%s_info_%s_freq_%s_div_%s_enc_%s_%s" %(direct_name, salary_prototype, difficulty_prototype, lambda_info, lambda_freq, lambda_div, lambda_enc, env_params))
                else:
    ...
    def transform_train_batch(self, data_batch, batch_size):
        ...
        # How to sample representative frequent sets?
        # Do we need sample?
        data_frequency = []
        for u in data_batch:
            data_frequency += u[3]
    ...
    def retrain_decoder(self):
        names = {'qnet_salary_embedding', 'qnet_salary_skill_action_value', 'qnet_salary_prototype', 'qnet_difficulty_embedding', 
        'qnet_difficulty_skill_action_value', 'qnet_difficulty_prototype'}

        for name in names:
            embedding = tf.Variable(self.Qnet.weights[name], name=name, trainable=False)
            self.Qnet.weights[name] = embedding
    ...
    def get_mcts_embeddings(self, part):
        mcts_file = str("wo_trans_" + "info_" + str(lambda_info) + "_freq_" + str(lambda_freq) + "_skill_sets" + '(' + part + ')')
...
parser.add_argument('--lambda_info', type=float, default=0.00)
...
if __name__ == "__main__":
    ...
    Qa = SeSRDQN_wo_trans(n_skill, lambda_info = lambda_info, lambda_div= lambda_div, lambda_enc = lambda_enc, lambda_freq = lambda_freq, verbose=1, learning_rate=lr, lambda_d=env_params['lambda_d'], embsize=emb_size, 
    ...
    # on_trainer.retrain_decoder()
    # on_trainer.train(n_batch=40000, batch_size=batch_size, data_train=data_train, data_valid=data_valid, verbose_batch=2048, T=20, target_update_batch=64)

    # ----------------- Re-train ---------------------
    # on_trainer.replace_prototype_embeddings()
    # print(Qa.lambda_freq)
    # quit()
    # on_trainer.train(n_batch=120000, batch_size=batch_size, data_train=data_train, data_valid=data_valid, verbose_batch=2048, T=20, target_update_batch=64)
    ...
    # ----------------- Model Save -----------------------
    Qa.save(HOME_PATH + "data/model/%s_SeSRDQN_wo_trans_sal_%s_dif_%s_info_%s_freq_%s_div_%s_enc_%s_%s" %(direct_name, salary_prototype, difficulty_prototype, lambda_info, lambda_freq, lambda_div, lambda_enc, env_params))