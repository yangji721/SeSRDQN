...
from Utils.Functions import evaluate, evaluate_with_example
...
if __name__ == "__main__":
    ...
    # ----------------- Model Training ---------------------
    short_trainer = ShortTrainer(Qa, train_samples=train_samples, environment=environment, pool_size=env_params['pool_size'], md=md)
    Qa.load(HOME_PATH + "data/model/%s_DNN_%s_%s" % (direct_name, md, env_params))

    # short_trainer.train(n_batch=40000, verbose_batch=10000, batch_size=256, data_train=data_train, data_test=None)
    ...
    # ----------------- Model Saving -----------------------
    # Qa.save(HOME_PATH + "data/model/%s_DNN_%s_%s" % (direct_name, md, env_params))
