...
class OnPolicyTrainer(object):
    ...
    def train(self, n_batch, batch_size, data_train, data_valid, save_path, verbose_batch=500, T=26):
        ...
        for it in tqdm(range(n_batch)):
            ...
            for t in range(T):
                ...
                self.memory.store((state_pre, s, (easy, salary - salary_pre)))  # Need to forget!!!
                salary_pre = salary
    ...
    def transform_train_batch(self, data_batch):
        # Input the data of a batch, of the form [(i, j)], return data_state, data_skill, data_q
...
if __name__ == "__main__":
    ...
    lambda_d, beta, pool_size = float(sys.argv[3]), float(sys.argv[4]), int(sys.argv[5])
    ...
    # ----------------- Model Reading ---------------------
    # Qa.load(HOME_PATH + "data/model/%s_DNN_%s_exp_%s" % (direct_name, md, env_params))
    ...
    on_trainer.train(n_batch=400000, batch_size=64, data_train=data_train, data_valid=data_valid, verbose_batch=10240,
    ...
    # ----------------- Model Saving -----------------------
    Qa.save(HOME_PATH + "data/model/%s_DNN_%s_exp_%s" % (direct_name, md, env_params))
