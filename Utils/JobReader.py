...
def skill_set_process(skill_set):
    '''
    :param skill_set: skill set, like {level: understanding, skill: lifecycle}
    :return: skill_list
    '''
...
def salary_process(salary_str):
    # Number of processed sequences (K RMB)
...
def itemset_process(skill_cnt):
    ...
    for itemset, frequency in tqdm(data):
        ...
        itemset.sort(key=lambda x: skill_cnt[x], reverse=True) # Descending order of occurrence
...
def get_frequent_itemset(a, b):
    with open(HOME_PATH + "data/item_set.pkl", "rb") as f:
        data = pickle.load(f) 
    result = []
    for itemset, frequency in data:
        if len(itemset) >= a and len(itemset) <= b:
...
def read_jd():
    '''
    :param path: jd_expanded.csv storage address
    :param dict_path: address of the skill number store
    :return: sample_lst: list(list, salary) form, each sample is a list of skills and corresponding salary
    '''
    path = JD_PATH
    df = pd.read_csv(path)[['id', 'skill_set', 'job_salary']].sort_values(by='id', ascending=True)
    n_sample = df.shape[0]
    assert n_sample == df['id'].max() + 1, "There is an error in the numbering of the job posting, the id maximum should be equal to the sample number - 1"
    sample_lst = []
